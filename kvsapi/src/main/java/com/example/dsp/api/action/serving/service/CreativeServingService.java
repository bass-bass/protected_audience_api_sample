...
@Component
public class CreativeServingService {
	private static final Logger logger = Logger.getLogger(CreativeServingService.class.getSimpleName());
	@Autowired
	private ServingLogic logic;

	public ResponseEntity<String> execute(ServingForm form) throws IOException {
		ResponseEntity<String> response = logic.makeCreativeTag(form);
